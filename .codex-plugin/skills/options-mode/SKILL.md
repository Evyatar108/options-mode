...
Use `/options-mode on`, `/options-mode off`, `/options-mode strict`, `/options-mode auto`, or `/options-mode status` to control options mode in Claude Code. Use `/options-mode default on|off|strict|auto|clear|status` to manage the global default stored in `<configRoot>/options.json` (per-session flags still override it).
...
- `auto` — builds on `strict`; the `no-question` tag is **not** valid. Every turn must end with an `AskUserQuestion` call (the PreToolUse hook intercepts it and auto-replies "user isn't here"), `<options-mode>task-complete</options-mode>` (clean done signal), or a background tag. Use for unattended sessions where the model continues autonomously (added v0.16.0).
